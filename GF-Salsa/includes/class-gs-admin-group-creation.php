...
class GFAdminGrps extends \GF_Field
{
   public $type = 'admin_groups';

    public function get_form_editor_field_title() {
        return esc_attr__( 'Admin Groups', 'wdm-gf-salsa' );
    }
   public function get_form_editor_field_settings() {
    ...
    public function get_checkbox_choices( $value, $disabled_text, $form_id = 0 ) {
        ...
        if ( is_array( $this->choices ) ) {
            ...
            if ( $count < $total ) {
                $choices .= "<li class='gchoice_total'>" . sprintf( esc_html__( '%d of %d items shown. Edit field to view all', 'wdm-gf-salsa' ), $count, $total ) . '</li>';
            }
        }
